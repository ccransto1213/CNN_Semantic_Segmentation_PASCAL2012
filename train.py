...
def getClassWeights():
    ...
    for _, mask in train_loader: #loop through every mask in train set
        for id in range(num_classes):
            class_inst[id]+=torch.sum(mask==id) #count number of class instances in each mask
        total_pixels+=mask.numel() #sum total number of pixels in each mask
    class_freq=class_inst/total_pixels #get class frequencies 
    eps=1e-8
    #Try median weight frequency
    #median_freq=torch.median(class_freq)
    #class_weights=median_freq/(class_freq+eps)

    #Try power weight frequency
    pow=0.6
    class_weights=(1.0/(class_freq+eps))**pow #Invert frequencies for class weights, lower frequency higher weight
    class_weights=class_weights/torch.sum(class_weights)*num_classes #normalize so they sum to 21
    print("Class frequencies:", class_freq)
    print("Class weights:", class_weights)
    print("Min weight:", torch.min(class_weights))
    print("Max weight:", torch.max(class_weights))
    print("Weight ratio (max/min):", torch.max(class_weights)/torch.min(class_weights))

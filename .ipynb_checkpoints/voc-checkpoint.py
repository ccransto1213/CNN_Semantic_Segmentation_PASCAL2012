...
class VOC(data.Dataset):
    ...
    def __init__(self, mode, transform=None, target_transform=None, both_transform=None):
        ...
        self.width = 224
        self.height = 224
        self.dimension_tuple = (224, 224)

    def __getitem__(self, index):

        img_path, mask_path = self.imgs[index]
        img = Image.open(img_path).convert('RGB').resize(self.dimension_tuple)
        mask = Image.open(mask_path).resize(self.dimension_tuple)

        ## Random Resized Crop
        # i, j, h, w = transforms.RandomResizedCrop.get_params(img, scale=(0.7, 1.0), ratio=(0.75, 1.33))
        # # img = transforms.functional.crop(img, i, j, h, w).resize(self.dimension_tuple)
        # # mask = transforms.functional.crop(mask, i, j, h, w).resize(self.dimension_tuple)
        # img = img.crop((j, i, j + w, i + h)).resize(self.dimension_tuple, Image.BILINEAR)
        # mask = mask.crop((j, i, j + w, i + h)).resize(self.dimension_tuple, Image.NEAREST)
